...
export default class AvatarEditorPage extends Page {
  ...
  oncreate(vnode) {
    ...
    app.setTitle(app.translator.trans('shebaoting-avatar-decoration.forum.editor.title'));
    app.setTitleCount(0);
  }
        ...
        <div className="AvatarDecorationPage">
          <div className="container">
            <div className="AvatarDecorationSignedOut">
              <h2>{app.translator.trans('shebaoting-avatar-decoration.forum.editor.sign_in_title')}</h2>
          ...
          <div className="AvatarDecorationShell">
              ...
              <div className="AvatarDecorationPreviewSticky">
                <div className="AvatarDecorationPreviewHeader">
                  <LinkButton
                    ...
                    aria-label={app.translator.trans('shebaoting-avatar-decoration.forum.editor.back')}
                    title={app.translator.trans('shebaoting-avatar-decoration.forum.editor.back')}
                  />
                  <h1>{app.translator.trans('shebaoting-avatar-decoration.forum.editor.title')}</h1>
                </div>
                ...
                <div className="AvatarDecorationActions">
                  <Button className="Button Button--primary" icon="fas fa-save" loading={this.saving} onclick={() => this.save()}>
                    {app.translator.trans('shebaoting-avatar-decoration.forum.editor.save')}
                  </Button>
                  <Button className="Button" icon="fas fa-undo" disabled={selectionCount(this.decoration) === 0} onclick={() => this.reset()}>
                    {app.translator.trans('shebaoting-avatar-decoration.forum.editor.reset')}
                  </Button>
                </div>
              </div>
            </aside>
            ...
            <main className="AvatarDecorationWardrobe">
              ...
              <div className="AvatarDecorationWardrobeHeader">
                ...
                >
                  {app.translator.trans('shebaoting-avatar-decoration.forum.editor.clear')}
                </Button>
              </div>
  ...
  colorRow(key) {
    ...
    return (
      <div className={classList('AvatarDecorationColorRow', key === 'background' && 'AvatarDecorationColorRow--background')} key={key}>
        <span>{app.translator.trans(`shebaoting-avatar-decoration.forum.editor.colors.${key}`)}</span>
  ...
  tabLabel(tab) {
    return app.translator.trans(`shebaoting-avatar-decoration.forum.editor.tabs.${tab.key}`);
  }
  ...
  save() {
    ...
    app.session.user
      ...
      .then((user) => {
        ...
        app.alerts.show({ type: 'success' }, app.translator.trans('shebaoting-avatar-decoration.forum.editor.saved'));
      })
      .catch((error) => {
        app.alerts.show(error.alert || { type: 'error' }, app.translator.trans('shebaoting-avatar-decoration.forum.editor.save_error'));
      })
      .finally(() => {
        this.saving = false;
        m.redraw();
  ...
  loadManifest() {
    ...
    return app
      .request({
        method: 'GET',
        url: `${app.forum.attribute('apiUrl')}/avatar-decoration/assets`,
      })
      .then((payload) => {
      ...
      .catch(() => {
        app.alerts.show({ type: 'error' }, app.translator.trans('shebaoting-avatar-decoration.forum.editor.load_error'));
      })
      .finally(() => {
        this.loading = false;
        m.redraw();
